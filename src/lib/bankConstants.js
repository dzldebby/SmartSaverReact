...
const INCREMENT = 5000; // Minimum increment for distribution

const REQUIREMENTS = {
  salary: (reqs) => reqs.hasSalary && reqs.salaryAmount >= 0,
  spend: (reqs) => reqs.spendAmount >= 500,
  giro: (reqs) => reqs.giroCount >= 3,
  insurance: (reqs) => reqs.hasInsurance,
  investments: (reqs) => reqs.hasInvestments,
  increasedBalance: (reqs) => reqs.increasedBalance
};
